...
Процедура ЗагрузкаДокументовССайта(ПараметрыОбмена)
	...
	Если Не Успешно Тогда
		Б_ОбменССайтомСервер.СообщитьПодробно("Авторизация на сайте не выполнена.", ПараметрыОбмена);	
		Возврат;
	КонецЕсли;
	...
	Пока лЕстьДокументы Цикл
		...
		ОтветСервера = Б_ОбменССайтомСервер.ПолучитьДанныеССервера(Соединение, АдресДляРаботы + НастройкиПодключения.ПараметрЗапросаHTTP_ПолучитьДанные + Токен, ЗаголовкиЗапросов, ПараметрыОбмена);
...
Функция ЗагрузитьДокументы(ДанныеЗаказов, ПараметрыОбмена)
	...
	ИначеЕсли НЕ Б_ОбменССайтомСервер.ОбъектXDTOСодержитСвойство(ДокументыXDTO, "Документ") Тогда
	...
	МассивДокументовXDTO = Б_ОбменССайтомСервер.ПолучитьМассивОбъектовСпискаXDTO(ДокументыXDTO.Документ);
...
Функция СоздатьОбновитьТиповыеДокументы(ПараметрыОбмена, ТзнДокументов)
	...
	Для каждого ТекДокумент из ТзнДокументов Цикл
		...
		Если Документ.Пустая() тогда	
			...
			ЗапросЗ.УстановитьПараметр("Идентификатор"	, ТекДокумент.Ид);
			ЗапросЗ.УстановитьПараметр("ТипДокумента"	, ТекДокумент.ХозОперация);
...
Процедура СоздатьОбновитьДокумент(ПараметрыОбмена, НазваниеДокумента, ДокументXML, НовыйДокумент, СсылкаНаДокумент = Неопределено)
	...
	ИначеЕсли НовыйДокумент И ДокументXML.ПометкаУдаления = ИСТИНА тогда
		Б_ОбменССайтомСервер.СообщитьПодробно("Не найден документ " + НазваниеДокумента + " с идентификаатором:" + ДокументXML.Ид + " . Документ не будет отменен.", ПараметрыОбмена);
		Возврат;   
	Иначе             
...
Функция ПроверкаНаПроведениеПоДополнительнымУсловиям(НазваниеДокумента, ДокументОтменен, ДокументXML, Документ)
	
	ПроводитьДокумент = Ложь;
	
	Если НазваниеДокумента = "РеализацияТоваровУслуг" И НЕ ДокументОтменен тогда
	...
	ИначеЕсли (НазваниеДокумента = "ПриходныйКассовыйОрдер" ИЛИ НазваниеДокумента = "ОперацияПоПлатежнойКарте") И НЕ ДокументОтменен тогда
	...
	ИначеЕсли (НазваниеДокумента = "ПоступлениеБезналичныхДенежныхСредств") И НЕ ДокументОтменен тогда
...
Процедура ОбновлениеСвойствДокументов(Документ, ДокументXML, ТипДокумента)
...
Процедура ЗаписьСтатусовИУдалениеЗагруженныхДокументов(ПараметрыОбмена, ДокументXML, ОсновныеНастройкиДокумента, Документ, НовыйДокумент, ДокументОтменен)
	...
	ОбновлениеСвойствДокументов(Документ, ДокументXML, ОсновныеНастройкиДокумента);
...
Функция ПолучитьЗначениеРеквизитаДокументаПоСоответствию(ПараметрыОбмена, ДокументXML, Документ, ОсновныеНастройкиДокумента, ТекущийРеквизит, СтрокаXML = Неопределено, СтрокаТабличнойЧасти = Неопределено)
	...
	Если ТекущийРеквизит.ТипДанных = "Из поля XML документа" тогда
		...
		ИначеЕсли ТекущийРеквизит.Значение = "Статус" тогда
			...
			Результат = ПолучитьСтатусДокумента(ПараметрыОбмена, ДокументXML);
		...
		ИначеЕсли ТекущийРеквизит.Значение = "Партнер" тогда
			...
			Результат = ПолучитьОбновитьПартнера(ПараметрыОбмена, Контрагент, ДокументXML.Контрагент);
		...
		ИначеЕсли ТекущийРеквизит.Значение = "Адрес контрагента" тогда
			...
			Если ДокументXML.Контрагент.Контакты <> Неопределено тогда
				...
				НайденныйАдрес = ДокументXML.Контрагент.Контакты.Найти("Адрес", "ТипВидКонтактнойИнформации");
				
				Если НайденныйАдрес <> Неопределено тогда 
					Результат =  СтрокаПоСпискуПолей(НайденныйАдрес.АдресныеПоля);
		...
		ИначеЕсли ТекущийРеквизит.Значение = "Эквайринговый терминал" тогда
			...
			Если ПараметрыОбмена.СоответствиеПлатежныхСистемИКасс <> Неопределено тогда
				...
				Если НайденнаяСтрока = Неопределено тогда
					Результат = Справочники.ЭквайринговыеТерминалы.ПустаяСсылка();
				Иначе
					Результат = ПараметрыОбмена.СоответствиеПлатежныхСистемИКасс.Найти(ИдОплаты, "ИдПлатежнойСистемы").ЭквайринговыйТерминал;
				КонецЕсли;
			КонецЕсли;
...
Функция ПолучитьСклад(ПараметрыОбмена, ДокументXML)
	...
	Если НЕ лСклад = Неопределено тогда
		Возврат лСклад;
	КонецЕсли;
...
// Возвращаемое значение:
//   Статусдокумента 
Функция ПолучитьСтатусДокумента(ПараметрыОбмена, ДокументXML)
	...
	Если ДокументXML.ХозОперация = "Заказ товара" тогда
		...
		Иначе
			...
			НайденныйСтатус = ПараметрыОбмена.СоответствиеСтатусовЗаказов.Найти(ИдСтатуса);
	...
	ИначеЕсли ДокументXML.ХозОперация = "Отпуск товара" тогда
		
		ИдСтатуса		= ПолучитьЗначениеРеквизитаДокумента(ДокументXML.ЗначенияРеквизитов, "Статус отгрузки");
	
		Если ИдСтатуса 	= Неопределено тогда
			СтатусДокумента = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате;	
		Иначе
			
			НайденныйСтатус = ПараметрыОбмена.СоответствиеСтатусовОтгрузок.Найти(ИдСтатуса);
			
			Если НайденныйСтатус <> Неопределено тогда
				Результат = НайденныйСтатус.Статус;
			Иначе
				Результат = Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате;	
			КонецЕсли;
...
Функция ПолучитьИнформациюОНоменклатуреДляДокумента(ПараметрыОбмена, СтрокаТовара)
	...
	Результат.Вставить("ЕдиницаИзмерения"			, ?(Номенклатура <> Неопределено, Номенклатура.ЕдиницаИзмерения, Неопределено));
...
Функция ПолучитьКонтрагента(ПараметрыОбмена, ЮрФизЛицо, ДокументXML)
	...
	Контрагент = Б_ОбменССайтомСервер.ПолучениеКонтрагентаПоКоду(ИнформацияОКонтрагенте.Ид, ИнформацияОКонтрагенте.Наименование);
	
	Если Контрагент = Неопределено тогда
		
		Если НЕ ПараметрыОбмена.СпособИдентификацииКонтрагентов = "Не использовать" Тогда
			...
			Если ПараметрыОбмена.СпособИдентификацииКонтрагентов = "ИНН" И ИнформацияОКонтрагенте.ЮрФизЛицо <> Перечисления.ЮрФизЛицо.ФизЛицо Тогда
