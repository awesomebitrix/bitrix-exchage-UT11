...
Функция ВыгрузитьКонтрагентаXDTO(ПараметрыОбмена, XDTOКонтрагентТип, Контрагент, МенеджерВременныхТаблицКонтрагентов)
	...
	Запрос.УстановитьПараметр("Владелец", Справочники.Контрагенты.ПустаяСсылка());
	...
	Запрос.УстановитьПараметр("Владелец", Справочники.Партнеры.ПустаяСсылка());
...
Процедура ВыгрузитьСкладыXDTO(ПараметрыОбмена, Владелец, ВладелецТип, МенеджерВременныхТаблиц)
	...
	XDTOСкладыТип	= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПараметрыОбмена.ФайлСхемы.Получить("Классификатор"), "Склады");	
	...
	Иначе	
		...
		Пока ВыборкаСкладов.Следующий() Цикл			
			...
			// РЧК. Добавляем информацию по магазинам.
			КонтактнаяИнформацияТаблицаЗначений = ВыборкаСкладов.Склад.КонтактнаяИнформация.выгрузить(, "Вид, Представление");
			
			КонтактнаяИнформацияСоответствие = Новый Соответствие;
			
			Для Каждого Строка Из КонтактнаяИнформацияТаблицаЗначений Цикл 
				СтрокаТаблицы = ОбщегоНазначения.СтрокаТаблицыЗначенийВСтруктуру(Строка);
				
				Если СтрокаТаблицы.Вид.Наименование = "Адрес склада" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "ADDRESS") Тогда
					XDTOСклад.ADDRESS = СтрокаТаблицы.Представление;
				ИначеЕсли СтрокаТаблицы.Вид.Наименование = "GPS_N" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "GPS_N")Тогда
					XDTOСклад.GPS_N = СтрокаТаблицы.Представление;
				ИначеЕсли СтрокаТаблицы.Вид.Наименование = "GPS_S" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "GPS_S")Тогда
					XDTOСклад.GPS_S = СтрокаТаблицы.Представление;
				ИначеЕсли СтрокаТаблицы.Вид.Наименование = "Телефон склада" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "PHONE")Тогда
					XDTOСклад.PHONE = СтрокаТаблицы.Представление;
				ИначеЕсли СтрокаТаблицы.Вид.Наименование = "График работы" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "SCHEDULE")Тогда
					XDTOСклад.SCHEDULE = СтрокаТаблицы.Представление;
				ИначеЕсли СтрокаТаблицы.Вид.Наименование = "Электронная почта" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "EMAIL") Тогда					
					XDTOСклад.EMAIL = СтрокаТаблицы.Представление;
				ИначеЕсли СтрокаТаблицы.Вид.Наименование = "Описание" и ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(XDTOСклад, "DESCRIPTION") Тогда
					XDTOСклад.DESCRIPTION  = СтрокаТаблицы.Представление;
				КонецЕсли;	
			КонецЦикла;
...
Процедура ВыгрузитьЕдиницыИзмеренияXDTO(ПараметрыОбмена, Владелец, ВладелецТип, МенеджерВременныхТаблиц)
	...
	XDTOЕдиницыИзмеренияТип	= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПараметрыОбмена.ФайлСхемы.Получить("Классификатор"), "ЕдиницыИзмерения");	
...
Процедура ВыгрузитьСоглашенияXDTO(ПараметрыОбмена, Владелец, ВладелецТип, МенеджерВременныхТаблиц)
	...
	XDTOТипыЦенТип	= ПолучитьТипСвойстваОтТипаОбъектаXDTO(ПараметрыОбмена.ФайлСхемы.Получить("Классификатор"),  "ТипыЦен");	
	XDTOТипыЦен 	= ФабрикаXDTO.Создать(XDTOТипыЦенТип); 
...
Функция СформироватьВременныеТаблицыПоИнформацииОТоварахПакета(ПараметрыОбмена, ИдентификаторКаталога, СписокТоваров, СписокСвойств)
	...
	|	ЕдиницыИзмерения.Ссылка В(&СписокЕдиниц)
	|	И ЕдиницыИзмерения.Ссылка <> ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|	И ЕдиницыИзмерения.Код ПОДОБНО ""%[0-9]%""
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЕдиницаИзмерения
	|;
...
Процедура ВыгрузитьТоварыXDTO(ПараметрыОбмена, ИмяКаталога, Владелец, ВладелецТип, МенеджерВременныхТаблиц, СтрокаТаблицыКаталогов, МассивКартинок, МассивФайлов)
	...
	Если ТзнТоваров.Количество() > 0 Тогда	
		...
		Для Каждого ТекТовар из ТзнТоваров Цикл
			...
			XDTOТовар.БазоваяЕдиница	= СокрЛП(XMLСтрока(ТекТовар.ИдентификаторЕдиницыИзмерения));
			
			// РЧК. Добавляем выгрузку коэффициента базовой единицы
			Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекТовар.ЕдиницаИзмерения, "НаименованиеПолное") = "Килограмм" Тогда
				XDTOТовар.КоэффициентЕдиницы = "0.1";
			Иначе
				XDTOТовар.КоэффициентЕдиницы = "1"
			КонецЕсли;
			///
...
Процедура ВыгрузитьПрайсы(ПараметрыОбмена, Владелец, ВладелецТип, лМенеджерВременныхТаблиц)
	...
	Если ТзнЦен.Количество() > 0 Тогда
		...
		Для НомерСтроки = 0 по  КоличествоСтрок Цикл
			...
			// РЧК. Проверка на правильность Коэффициента. Передать на уровень запроса
			Если Коэффициент <> Null Тогда 
				ЦенаТовара 			= ТекЦена.Цена/Коэффициент;
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Для товара не задан коэффициент, цена товара выгружена без учета коэффициента.'"));
				ЦенаТовара 			= ТекЦена.Цена;
			КонецЕсли;
...
Функция ВыгрузитьОтгрузкуXDTO(ПараметрыОбмена, МенеджерВременныхТаблиц, Документ, МассивТоваров, КоличествоОтгрузок)
	...
	Если ЕстьТовары И Не Документ.Отменен Тогда
		...
		Если ЕстьТовары Тогда
			...
			Если Число(Документ.СуммаДоставки) > 0 Тогда
...
Процедура ЗаполнениеТабличнойЧастиДокумента(ПараметрыОбмена, МенеджерВременныхТаблиц, Владелец, ВладелецТип, Документ, МассивНоменклатуры)
	...
	Для Каждого ТекНоменклатура из МассивНоменклатуры Цикл 	
		...
		ДобавитьСвойстваПозицийТовараДокумента(ПараметрыОбмена, МенеджерВременныхТаблиц, XDTOЗначенияРеквизитов, XDTOЗначениеРеквизитаТип, ТекНоменклатура);
...
Процедура ДобавитьСвойстваПозицийТовараДокумента(ПараметрыОбмена, МенеджерВременныхТаблиц, XDTOЗначенияРеквизитов, XDTOЗначениеРеквизитаТип, ПозицияТовара);
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Текст = "ВЫБРАТЬ
	|	ВремСвойстваТоваровДокументов.НаименованиеСвойства,
	|	ВремСвойстваТоваровДокументов.Значение
	|ИЗ
	|	ВремСвойстваТоваровДокументов КАК ВремСвойстваТоваровДокументов
	|ГДЕ
	|	ВремСвойстваТоваровДокументов.Документ = &Документ
