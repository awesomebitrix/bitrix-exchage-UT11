...
Функция ВыполнениеLongPullingРекурсивно(ПараметрыОбмена, НастройкиПодключения)
	...
	Исключение
		...
		СообщитьПодробно("Не удалось получить данные с сервера на этом подключении в реал-тайм обмене. ", ПараметрыОбмена)
...
// Функция - отправляет http запрос для деактивации данных на сайте.
//
// Параметры:
//  ПараметрыОбмена	 - 	 Настройки узла обмена  
...
Функция ОбновлениеДоВерсии5_0_0_1(ОсновныеПараметры)
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 5.0.0.1 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры);
		Возврат Ложь;
...
Функция ОбновлениеДоВерсии5_0_0_7(ОсновныеПараметры)
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 5.0.0.7 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры);
		Возврат Ложь;
...
Функция ОбновлениеДоВерсии6_0_0_1(ОсновныеПараметры)
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 6.0.0.1 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры);
		Возврат Ложь;
...
Функция ОбновлениеДоВерсии6_0_1_2(ОсновныеПараметры)
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 6.0.1.2 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры);
		Возврат Ложь;
...
Функция ОбновлениеДоВерсии6_0_3_1(ОсновныеПараметры)
	...
	Исключение
		...
		СообщитьПодробно("Обновление до версии 6.0.3.1 завершено с ошибками. Продолжение обмена невозможно.", ОсновныеПараметры);
		Возврат Ложь;
...
Функция ДобавитьОсновныеНастройкиВПараметры(УзелОбмена, Параметры, ПринудительнаяПолноеОбновлениеДанныхТовара = Ложь) Экспорт
	...
	Параметры.Вставить("ВерсияСхемы"		, "2.10");
...
Функция ДобавитьНастройкиОбменаВПараметры(УзелОбмена, Параметры) Экспорт
	...
	Если Параметры.ОбменТоварами Тогда
		...
		ТаблицыКлассификаторовНоменклатуры 	= Параметры.ХранилищеТаблицыКлассификаторовНоменклатуры.Получить();
		Параметры.Вставить("ТаблицыКлассификаторовНоменклатуры"	, ТаблицыКлассификаторовНоменклатуры);

		ТаблицаКаталогов = ПолучитьТаблицуКаталогов(Параметры);
		
		ДобавитьКлассификаторыВТаблицуКаталогов(ТаблицаКаталогов, Параметры.ТаблицыКлассификаторовНоменклатуры);
...
Процедура ВыполнитьОбмен(Настройка, ПринудительнаяПолноеОбновлениеДанныхТовара = Ложь) Экспорт
	...
	НастройкиУспешноДобавлены = ДобавитьОсновныеНастройкиВПараметры(Настройка, ОсновныеПараметры, ПринудительнаяПолноеОбновлениеДанныхТовара);
	...
	Если ОсновныеПараметры.ВыполнятьПолнуюВыгрузкуПринудительно ИЛИ ОсновныеПараметры.ВыгружатьНаСайт тогда
		...
		Если ВремТзн.Количество()=0 Тогда
			...
			Если ОсновныеПараметры.ВыгружатьНаСайт тогда
				ПланыОбмена.УдалитьРегистрациюИзменений(ОсновныеПараметры.Ссылка);
			КонецЕсли;
...
Процедура ДобавитьКлассификаторыВТаблицуКаталогов(ТаблицаКаталогов, ТаблицаКлассификаторов) Экспорт
	
	ТаблицаКаталогов.Колонки.Добавить("ДеревоГрупп", Новый ОписаниеТипов("ДеревоЗначений"));
	
	Если ТаблицаКлассификаторов = НеОпределено Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицыКлассификаторов Из ТаблицаКлассификаторов Цикл
		
		Если ПустаяСтрока(СтрокаТаблицыКлассификаторов.ИдентификаторКаталога) Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаКаталога = ТаблицаКаталогов.Найти(СтрокаТаблицыКлассификаторов.ИдентификаторКаталога, "ИдентификаторКаталога");
		
		Если СтрокаКаталога = НеОпределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаКаталога.ДеревоГрупп = СтрокаТаблицыКлассификаторов.ДеревоГрупп;
