...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЭтоАдресВременногоХранилища(АдресТаблицыКлассификаторовНоменклатуры) тогда
		ТаблицыКлассификаторовТЗ = ПолучитьИзВременногоХранилища(АдресТаблицыКлассификаторовНоменклатуры);
		ТекущийОбъект.ХранилищеТаблицыКлассификаторовНоменклатуры = Новый ХранилищеЗначения(ТаблицыКлассификаторовТЗ);
	КонецЕсли;	
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ВыгружатьНаСайт И НЕ ЗначениеЗаполнено(Объект.АдресСайта) тогда
		...
		Сообщить("Адрес сайта не указан. Сохранить настройку нельзя.");
		Отказ = Истина;
	...
	Если НЕ Объект.ВыгружатьНаСайт И НЕ ЗначениеЗаполнено(Объект.КаталогВыгрузки) тогда
		...
		Сообщить("Каталог выгрузки не указан. Сохранить настройку нельзя.");
		Отказ = Истина;
...
Функция ПолучитьНастройкиВерсионности()
	...
	Если НЕ ЗначениеЗаполнено(лСпкУстановкаВерсий) тогда
		...
		лСпкУстановкаВерсий.Добавить("Товаров");
...
Процедура ПослеЗакрытияВопросаОПринудительномОбмене(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Состояние(
		Б_ОбменССайтомКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ВыполнитьОбменДаннымиНаСервере(Объект.Ссылка, Истина);
...
Процедура НастроитьВыгрузкуИнфомациоОТоварах(Команда)
	...
	ПараметрыФормы.Вставить("АдресТаблицыКлассификаторовНоменклатуры", АдресТаблицыКлассификаторовНоменклатуры);
...
Процедура ОбновлениеНастроекСервер(ОтветФормы, ОбновлятьТаблицуКаталогов = Ложь)
	...
	Если ОбновлятьТаблицуКаталогов тогда
		...
		Если ЭтоАдресВременногоХранилища(ОтветФормы.АдресТаблицыКлассификаторовНоменклатуры) тогда
		
			лПеременная = ПолучитьИзВременногоХранилища(ОтветФормы.АдресТаблицыКлассификаторовНоменклатуры);
			АдресТаблицыКлассификаторовНоменклатуры = ПоместитьВоВременноеХранилище(лПеременная, УникальныйИдентификатор);
		КонецЕсли;	
	КонецЕсли;
...
Процедура ЗаполнитьТаблицуКаталоговСервер()
	...
	СохраненнаяТаблицыКлассификаторовНоменклатуры = РеквизитФормыВЗначение("Объект").ХранилищеТаблицыКлассификаторовНоменклатуры.Получить();
	
	Если ТипЗнч(СохраненнаяТаблицыКлассификаторовНоменклатуры) = Тип("ТаблицаЗначений") Тогда
		АдресТаблицыКлассификаторовНоменклатуры = ПоместитьВоВременноеХранилище(СохраненнаяТаблицыКлассификаторовНоменклатуры, УникальныйИдентификатор);
	КонецЕсли;
...
Функция ПолучитьНастройкиИнформацииТоваровПоУмолчанию()
	...
	Результат.Вставить("АдресТаблицыКлассификаторовНоменклатуры", Неопределено);
...
Функция ПолучитьНастройкиКонтрагентовПоУмолчанию()
	...
	Результат.Вставить("СпособИдентификацииКонтрагентов"			, "Наименование");	
...
Функция ПолучитьНастройкиЗаказовПоУмолчанию()
	...
	Массив = Новый Массив;
	Массив.Добавить(Тип("ПеречислениеСсылка.СтатусыЗаказовКлиентов"));
	ОписаниеТиповСЗ = Новый ОписаниеТипов(Массив);

	СоответствиеСтатусовЗаказов = Новый ТаблицаЗначений;
	СоответствиеСтатусовЗаказов.Колонки.Добавить("ИдСтатуса"		, ОписаниеТиповС);
	СоответствиеСтатусовЗаказов.Колонки.Добавить("Статус"			, ОписаниеТиповСЗ);
	СоответствиеСтатусовЗаказов.Колонки.Добавить("НазваниеСтатуса"	, ОписаниеТиповС);

	Результат.Вставить("СоответствиеСтатусовЗаказов", СоответствиеСтатусовЗаказов);	
	
	СоответствиеСтатусовОтгрузок = СоответствиеСтатусовЗаказов.СкопироватьКолонки();	
	
	Результат.Вставить("СоответствиеСтатусовОтгрузок", СоответствиеСтатусовОтгрузок);	
	...
	СоответствиеПлатежныхСистемИКасс.Колонки.Добавить("ЭквайринговыйТерминал"	, ОписаниеТиповЭТ);
...
Процедура ВыполнитьОбменДанными(Команда)
	...
	Иначе
		...
		Состояние(Б_ОбменССайтомКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 начат обмен данными с сайтом'"),	Формат(ТекущаяДата(), "ДЛФ=DT")),,
			Б_ОбменССайтомКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'по узлу обмена ""%1""...'"), Объект.Ссылка));
		
		ВыполнитьОбменДаннымиНаСервере(Объект.Ссылка, Ложь);
...
&НаСервере
Процедура ВыполнитьОбменДаннымиНаСервере(Узел, ТипВыгрузкуКартинок)
	Б_ОбменССайтомСервер.ВыполнитьОбмен(Узел, ТипВыгрузкуКартинок);    
КонецПроцедуры
